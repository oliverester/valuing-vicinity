...
class DataProvider():
    ...
    def __init__(self,
        ...
        if inference_dataset_type is None:
            self.inference_collate_fn = train_collate_fn
            self.inference_dataset_type = train_dataset_type
        else:
            self.inference_collate_fn = inference_collate_fn
    ...
    def get_train_loader(self):
        ...
        if self.train_set is not None:
            ...
            logger.info(f"Train Data set length {len(train_dataset)}")
    ...
    def get_wsi_loader(self,
        ...
        """
        wsi_pytorch_dataset = self.inference_dataset_type(wsi_dataset=wsi,
                                                          patches=patches,
                                                          transform=self.vali_aug_transform)
        # validation loader does not have distributed loader. So, each GPU runs a full validation run. However, only rank 0 prints
        wsi_loader = torch.utils.data.DataLoader(wsi_pytorch_dataset,
                                                 ...
                                                 num_workers=self.workers,
                                                 pin_memory=True,
                                                 collate_fn=self.inference_collate_fn,
...
class HoldoutSet():
    ...
    def _create_loader(self):
        ...
        logger.info(f"Train Data set length {self.train_wsi_dataset.metadata['n_drawn_patches']}"
            f" patches from {self.train_wsi_dataset.metadata['n_wsis']} wsis")
        logger.info(f"Vali Data set length {self.vali_wsi_dataset.metadata['n_drawn_patches']}"
        ...
        if self.test_wsi_dataset is not None:
            ...
            logger.info(f"Test Data set length {self.test_wsi_dataset.metadata['n_drawn_patches']}"
...
class McSet():
    ...
    def _create_mc_set(self):
        logger.info("Creating Monte-Carlo-Sets")
        runs = []
        ...
        for run in range(self.runs):
            logger.info(f"Monte-Carlo-Set {run}")
...
class CvSet():
    ...
    def _create_kfold_set(self):
        ...
        """
        logger.info("Creating Cross-Validation-Sets")
        folds = []
        wsi_labels = self.wsi_dataset.get_wsi_labels()
        wsis = self.wsi_dataset.get_wsis()
